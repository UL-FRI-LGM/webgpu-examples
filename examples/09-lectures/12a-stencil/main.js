...
const pipeline = device.createRenderPipeline({
    ...
    depthStencil: {
        ...
        depthCompare: 'less',
        stencilFront: {
            compare:'always', // always pass stencil test
            passOp:'replace', // set stencil to reference value on pass
        },
        stencilBack: {
            compare:'always', // always pass stencil test
            passOp:'replace', //
...
const codeOutline = await fetch('shader.outline.wgsl').then(response => response.text());
const moduleOutline = device.createShaderModule({ code: codeOutline });
const pipelineOutline = device.createRenderPipeline({
    ...
    depthStencil: {
        ...
        depthCompare: 'always',
        stencilFront: {
            compare: 'equal' // pass only where stencil equals reference value
        },
        stencilBack: {
             compare: 'equal' // pass only where stencil equals reference value
        }
...
const uniformBufferOutline = device.createBuffer({
    size: 64,
    usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
});
...
const bindGroupOutline = device.createBindGroup({
    layout: pipelineOutline.getBindGroupLayout(0),
    entries: [
        {
            binding: 0,
            resource: { buffer: uniformBufferOutline },
        },
    ],
});
...
function render() {
    ...
    var modelScaled = mat4.create()
    mat4.scale(modelScaled, modelMatrix, [1.05,1.05,1.05]);
    mat4.multiply(modelViewMatrix, viewMatrix, modelScaled);
    mat4.multiply(modelViewProjectionMatrix, projectionMatrix, modelViewMatrix);
    queue.writeBuffer(uniformBufferOutline, 0, modelViewProjectionMatrix);

    renderPass.setPipeline(pipelineOutline);
    renderPass.setBindGroup(0, bindGroupOutline);
    renderPass.setVertexBuffer(0, vertexBuffer);
    renderPass.setIndexBuffer(indexBuffer, 'uint32');
