...
const pipeline = device.createRenderPipeline({
    ...
    fragment: {
        ...
        targets: [{ format,
            blend: {
            // operation can be: "add", "subtract", "reverse-subtract", "min", "max"
            // factor can be: "zero", "one", "src", "one-minus-src", "src-alpha", "one-minus-src-alpha", "dst",
            // "one-minus-dst", "dst-alpha", "one-minus-dst-alpha", "src-alpha-saturated", "constant", "one-minus-constant",
                color: {
                    operation: 'add', // dst = 1 * src + 0 * dst
                    srcFactor: 'src-alpha',
